  ...
  hooks: {
    beforeChange: [
      async ({ data, originalDoc }) => {
        ...
        if (currentRefundStatus === 'requested' || currentRefundStatus === 'refunded') {
          data.status = 'cancelled'
        }
        if (
          currentRefundStatus === 'requested' &&
          !data.refundRequestedAt &&
          !originalDoc?.refundRequestedAt
        ) {
          data.refundRequestedAt = new Date().toISOString()
        }
        if (currentRefundStatus === 'refunded' && !data.refundedAt && !originalDoc?.refundedAt) {
    ...
    afterChange: [
      async ({ doc, previousDoc, operation, req }) => {
        ...
        // Send rejection email when status changes to 'declined' or 'cancelled' (unless refund requested or refunded)
